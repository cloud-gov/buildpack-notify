...
when each buildpack was last updated. It will find all the applications using the system buildpacks and compare the
current droplet creation time with the buildpack update time. If the current droplet predates the buildpack update,
it will queue all the space managers and space developers to receive an e-mail about that application. To prevent users
from receiving multiple e-mails, all the applications in violation are grouped per user so that the user receives one
e-mail notifying them about all the applications instead of an e-mail per application. After the notifications are sent
out, the buildpack version metadata (GUID and last updated time) is stored in the state. By storing that data,
notifications won't be sent out again when the cron job runs unless the buildpack is updated by system admins again.
...
- `SMTP_PASSWORD`: The password to authenticate with. e.g. `somepassword`
